...
import { isAdminPassword, signData, unsignData } from "../secrets";
...
export const auth = os
  ...
  .middleware(async ({ context, next }) => {
    ...
    if (!authHeader?.startsWith("Bearer ")) {
      throw new ORPCError("Invalid or missing authorization header");
    }
    ...
    if (session.sessionStart.getTime() + sessionTimeout < Date.now()) {
      ...
      throw new ORPCError("Session expired");
    }
    return next({
  ...
  createUserKey: os
    ...
    .handler(async ({ input }) => {
      if (!(await isAdminPassword(input.adminPassword))) {
        throw new ORPCError("Invalid admin password");
      }
